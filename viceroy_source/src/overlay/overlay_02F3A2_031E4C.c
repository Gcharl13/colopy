...
/* strcat_far is declared canonically in iolib.h (char far*(char far*,const char far*));
 * not re-declared here. @asm 0x0D1D:0x117E far strcat into display buf */
...
void report_header_row(int draw_arg)
{
    ...
    strcat_far(buf, (const char far *)"" /* num scratch */); /* @asm 0x030F93 0x0D1D:0x117E */
    ...
    strcat_far(buf, (const char far *)"" /* num scratch */); /* @asm 0x03101B 0x0D1D:0x07A4 */
...
void report_table_16rows(int input_enabled)
{
    ...
    for (i = 0; i < 0x10; i++) {                         /* @asm 0x031253 16 rows */
        ...
        strcat_far(buf, (const char far *)"" /* num scratch */); /* @asm 0x031139 0x0D1D:0x7A4 */
...
void unit_stack_panel(int input_enabled)
{
    ...
    if (G16(0x0FA2) == 0) {                              /* @asm 0x0314F1 LIST mode */
        ...
        for (slot = 0; slot < 6; slot++) {               /* @asm 0x031739 */
            ...
            q     = order_assign(slot /* TODO: verify arg */);  /* @asm 0x031791 0x181F:0xC68 = func_030C68 */
...
void naval_list_panel(int input_enabled)
{
    ...
    strcat_far((char far *)0 /* title buf */, (const char far *)"" /* num scratch */); /* @asm 0x0318F8 0x0D1D:0x117E */
...
int beveled_num_cell(int value, int x, int y, int mode)
{
    ...
    strcat_far(buf, (const char far *)"" /* num scratch */); /* @asm 0x031D51 0x0D1D:0x117E */
