...
void king_demand_cadence(int power_id)   /* ANCHOR_VERIFIED loop; trigger not yet decoded */
{
    ...
    if (power_id < 4 && g_ai_personality_543F[power_id * 0x34] == 0) {
         ...
         * selection probability itself is not yet decoded (overlay/data-driven). */
 ...
 * design, not for lack of trying; a full port would be UI plumbing over body in thunk page
