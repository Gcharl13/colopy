...
1. **The byte-verified C source** — `reverse_engineered/viceroy_source/src/**/*.c`
   ...
   `[not yet decoded]` banner is *not* tier-1 for the un-verified parts — read the per-file/
...
| `docs/COMBAT_STATS.md` | 2026-05-30 | @UNIT column → stat-offset map; per-unit atk/def table | itself + `src/combat/combat.c` + `data/unit_classes.c` | BYTE_VERIFIED. Resolves the `[RUNTIME_ONLY (data-resident)]` and RULINGS wave-6 §3. LAND atk=0x5236/def=0x5235; SHIP guns=0x523b/hull=0x523c. **This is the combat-stats authority** (overrides `docs/COMBAT.md`). |
...
| `docs/NEXT_TARGETS.md` | 2026-05-30 | Remaining unknown functions / residue after overlay port | itself | Current. Records "overlay C-port complete (0 orphans)" + the 12 bounded `not-yet-decoded-inner` functions grouped by root cause. A live backlog, not a claims doc. |
...
| `RECONSTRUCTION_PLAN.md` | 2026-05-30 | The plan / scope / END GOAL (C source = the product) | itself | Current. Encodes the 2026-05-30 user directives (byte-accuracy + portable-spec structure; cite-or-not-yet-decoded absolute). |
...
| `docs/COMBAT.md` | 2026-05-28 | Combat resolution model | `src/combat/combat.c` (BYTE_VERIFIED) + `docs/COMBAT_STATS.md` + RULINGS combat waves 6/7/9/10 | **Wrong RNG form + over-broad bonuses.** Uses `roll=random_range(0,total-1); roll<atk`; the verified roll is `random_int(1,ATK+DEF); attacker wins iff roll<=ATK` (RULINGS 2026-05-28 combat says the old form "breaks replay"). The flat +50% fortified/veteran/SoL multipliers were SCOPED by wave-6/9: the ship roll reads RAW stats (no scaler); land bonuses live in the func_05CA7E modifier chain (manual-confirmed but table still partly not yet decoded). Promotion/demotion ladder prose is close to `combat_demotion_ladder.c` but unverified in detail. *Mostly SUPERSEDED by `combat.c` + `COMBAT_STATS.md`; left as STALE-REVIEW because no single prose doc replaces its narrative scope.* |
...
2. **AI behaviour / personality math** — `docs/AI_SYSTEM.md` is contradicted;
   the personality-weighted decision math is still largely overlay-`not yet decoded`
...
4. **Colony production / building economics** — `docs/COLONY_SYSTEM.md` is
   ...
   production formulas remain partly `not yet decoded`.
5. **Native behavioural tables** (aggression / pop / skills) — explicitly
   `not yet decoded`/overlay-resident per RULINGS 2026-05-30; `data/tribe_data.c` carries
   only the verified @TRIBES/@LEVELS data, behavioural params demoted to not yet decoded.
6. **Market price→coin-value (bid/ask) curve** — `not yet decoded` behind overlay thunk
...
7. **Land combat terrain/fort bonus table** (feeds `[0x8D04]`) — manual-confirmed
   to exist (RULINGS 2026-05-30 manual) but the numeric table is still `not yet decoded`;
