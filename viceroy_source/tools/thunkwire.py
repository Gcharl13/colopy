...
def main():
        ...
        if ta > 0:
            ...
            if cas and cas != {ta}:
                report[f"arity mismatch (target wants {ta})"].append(
                    (stub_sym, tgt_sym, sorted(cas)))
                arity_blocked[stub_sym] = (tgt_sym, ta); return
            if not cas:
    ...
    # stubs whose target is RESOLVED but blocked only by call-site arity --
    # consumed by tools/argrestore.py to restore commented-out arguments
    json.dump({s: {"target": t, "arity": a} for s, (t, a) in arity_blocked.items()},
              open(os.path.join(GEN, "thunk_arity_blocked.json"), "w"), indent=1)
