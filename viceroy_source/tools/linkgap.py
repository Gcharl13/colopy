...
def main():
    ...
    # Infrastructure symbols defined in tools/stub_hits.c -- never stub them;
    # a weak stub for viceroy_stub_hit would create an infinite self-call.
    infra_syms = {"viceroy_stub_hit", "viceroy_stub_report"}
    stub_syms = [s for k in ("overlay_thunk", "func_body", "named_gap")
                 for (s, _) in buckets[k]
                 if s not in wired and s not in libc_syms and s not in infra_syms]
