...
Externally-undefined symbols in `libviceroy_rules.a`: **2198**
...
| func_body | 149 | port the original `func_0XXXXX` body |
| named_gap | 737 | implement, or re-include excluded render/ audio/ source |

Of 869 overlay thunks, **398** already have a resolved target name in `thunk_signatures.json` (ready to wire).
