...
Externally-undefined symbols in `libviceroy_rules.a`: **2298**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 466 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 906 | wire to resolved target, else port the overlay leaf |
| func_body | 185 | port the original `func_0XXXXX` body |
| named_gap | 741 | implement, or re-include excluded render/ audio/ source |

Of 906 overlay thunks, **406** already have a resolved target name in `thunk_signatures.json` (ready to wire).
...
## Mixed data/function usage — 14 symbols (porter inconsistency)
