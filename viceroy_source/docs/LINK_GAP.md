...
Externally-undefined symbols in `libviceroy_rules.a`: **2225**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 458 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 881 | wire to resolved target, else port the overlay leaf |
| func_body | 172 | port the original `func_0XXXXX` body |
| named_gap | 714 | implement, or re-include excluded render/ audio/ source |

Of 881 overlay thunks, **400** already have a resolved target name in `thunk_signatures.json` (ready to wire).
