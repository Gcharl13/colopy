...
| overlay_thunk | 893 | wire to resolved target, else port the overlay leaf |
| func_body | 183 | port the original `func_0XXXXX` body |
| named_gap | 740 | implement, or re-include excluded render/ audio/ source |

Of 893 overlay thunks, **403** already have a resolved target name in `thunk_signatures.json` (ready to wire).
