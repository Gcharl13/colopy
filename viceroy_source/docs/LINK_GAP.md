...
Externally-undefined symbols in `libviceroy_rules.a`: **2211**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 443 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 868 | wire to resolved target, else port the overlay leaf |
| func_body | 162 | port the original `func_0XXXXX` body |
| named_gap | 738 | implement, or re-include excluded render/ audio/ source |

Of 868 overlay thunks, **397** already have a resolved target name in `thunk_signatures.json` (ready to wire).
