...
| overlay_thunk | 905 | wire to resolved target, else port the overlay leaf |
| func_body | 185 | port the original `func_0XXXXX` body |
| named_gap | 742 | implement, or re-include excluded render/ audio/ source |

Of 905 overlay thunks, **406** already have a resolved target name in `thunk_signatures.json` (ready to wire).
