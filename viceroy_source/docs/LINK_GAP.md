...
| overlay_thunk | 878 | wire to resolved target, else port the overlay leaf |
| func_body | 156 | port the original `func_0XXXXX` body |
| named_gap | 736 | implement, or re-include excluded render/ audio/ source |

Of 878 overlay thunks, **400** already have a resolved target name in `thunk_signatures.json` (ready to wire).
