...
Externally-undefined symbols in `libviceroy_rules.a`: **2227**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 456 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 877 | wire to resolved target, else port the overlay leaf |
| func_body | 157 | port the original `func_0XXXXX` body |
| named_gap | 737 | implement, or re-include excluded render/ audio/ source |

Of 877 overlay thunks, **400** already have a resolved target name in `thunk_signatures.json` (ready to wire).
...
## Byte-exactness leak — 2 DGROUP globals defined in-lib
