# Modern-build LINK gap (auto-generated by tools/linkgap.py)

Externally-undefined symbols in `libviceroy_rules.a`: **2302**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 465 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 914 | wire to resolved target, else port the overlay leaf |
| func_body | 184 | port the original `func_0XXXXX` body |
| named_gap | 739 | implement, or re-include excluded render/ audio/ source |

Of 914 overlay thunks, **413** already have a resolved target name in `thunk_signatures.json` (ready to wire).

## Generated artifacts
- `tools/generated/linkfloor.ld` — byte-exact DGROUP-global aliases
- `tools/generated/linkfloor_stubs.c` — weak no-op floor for the rest

## Byte-exactness leak — 0 DGROUP globals defined in-lib

These carry a DGROUP-offset name but have a stray tentative/strong
definition in the source, so they get their OWN storage instead of
aliasing into `g_dgroup` — `memcmp` vs the original data segment will
diverge on them. Fix: change the definition to `extern` (or delete it)
so the `linkfloor.ld` alias takes over.


## Mixed data/function usage — 14 symbols (porter inconsistency)

Named like DGROUP data but CALLED like a function somewhere in
src/ — one file modeled the offset as a variable, another as a
getter. NOT aliased (a call would jump into the data segment);
each gets a weak stub until the two models are reconciled.

- `ai_path_budget_8DB8`
- `ai_turn_counter_538E`
- `colony_8542`
- `colony_field_C86`
- `colony_finalize_8DC6`
- `colony_helper_C22`
- `colony_helper_C5E`
- `colony_misc_b82`
- `ff_recognized_7B4`
- `g_iob_dispatch_2B18`
- `panel_helper_1E1D`
- `panel_helper_1E6D`
- `sol_helper_3695`
- `unit_translate_action_8BC6`

## named_gap detail (the real worklist — sample)

- `__aFldiv`
- `__aFlmul`
- `__errno_location`
- `__stack_chk_fail`
- `aFldiv`
- `aFldiv_0F92`
- `aFlmul`
- `aFlmul_0F60`
- `ai_path_budget_8DB8`
- `ai_set_plot_target`
- `ai_turn_counter_538E`
- `ai_turn_end_tail_03EA06`
- `assign_job`
- `atoi`
- `bars_begin`
- `bars_draw`
- `bells_for_player`
- `bios_kbd_shift_flags`
- `blit_band`
- `blit_box_id`
- `blit_sprite`
- `box_bevel`
- `box_clear`
- `box_fill`
- `box_frame`
- `box_outline`
- `box_show_191F_08F8_b`
- `building_finished_fx`
- `cargo_col0`
- `cargo_row_decode`
- `cc_bg_load_44E`
- `cc_blit_3F4`
- `cc_clip_3B6`
- `cc_done_AAC`
- `cc_fill_444`
- `cc_pal_3EA`
- `cc_present_E2`
- `cc_restore_3C0`
- `cd_colony_attr`
- `cd_colony_field`
- `cd_list_count`
- `cd_list_first_ok`
- `cd_snapshot`
- `cd_teardown`
- `cd_win_create`
- `checkbox_get`
- `checkbox_set`
- `classify_terrain`
- `close`
- `colonist_profession`
- `colony_8542`
- `colony_effect_id9`
- `colony_event_82`
- `colony_field_C86`
- `colony_finalize_8DC6`
- `colony_footer`
- `colony_growth_event`
- `colony_hdr_panel`
- `colony_helper_C22`
- `colony_helper_C5E`
