...
Externally-undefined symbols in `libviceroy_rules.a`: **2228**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 456 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 880 | wire to resolved target, else port the overlay leaf |
| func_body | 156 | port the original `func_0XXXXX` body |
| named_gap | 736 | implement, or re-include excluded render/ audio/ source |

Of 880 overlay thunks, **400** already have a resolved target name in `thunk_signatures.json` (ready to wire).
