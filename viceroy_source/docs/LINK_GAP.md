...
Externally-undefined symbols in `libviceroy_rules.a`: **2224**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 442 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 892 | wire to resolved target, else port the overlay leaf |
| func_body | 185 | port the original `func_0XXXXX` body |
| named_gap | 705 | implement, or re-include excluded render/ audio/ source |
