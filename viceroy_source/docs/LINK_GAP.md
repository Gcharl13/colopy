...
Externally-undefined symbols in `libviceroy_rules.a`: **2282**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 464 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 895 | wire to resolved target, else port the overlay leaf |
| func_body | 183 | port the original `func_0XXXXX` body |
| named_gap | 740 | implement, or re-include excluded render/ audio/ source |

Of 895 overlay thunks, **403** already have a resolved target name in `thunk_signatures.json` (ready to wire).
