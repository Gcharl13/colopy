...
Externally-undefined symbols in `libviceroy_rules.a`: **2303**

| category | count | how it closes |
|---|--:|---|
| dgroup_global | 465 | byte-exact `g_dgroup` alias (linkfloor.ld) — DONE on link |
| overlay_thunk | 915 | wire to resolved target, else port the overlay leaf |
| func_body | 184 | port the original `func_0XXXXX` body |
| named_gap | 739 | implement, or re-include excluded render/ audio/ source |

Of 915 overlay thunks, **414** already have a resolved target name in `thunk_signatures.json` (ready to wire).
