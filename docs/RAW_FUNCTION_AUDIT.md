...
| `0x078548` | 49 | `mcga_getpal` | B | VGA: **reads** the 256-colour palette back out of the DAC into the caller's far buffer — `out 0x3C7` is the *read* index (0x3C8 is write) and the transfer is `insb` @0x07857E, port→memory; vsync via IN 0x3DA; sets the DAC-busy lock [0x808]. **Corrected 2026-08-05** — was `vga_palette_dac_write` "writes the palette to the DAC", which noticed port 0x3C7 and drew the opposite conclusion. The name is MAPEDIT's CodeView `@mcga_getpal` (`mcga_8.c`), matched instruction for instruction; the writing counterpart is `@mcga_setpal_range` @`0x00E702` (`out 0x3C8` @0x00E730 then `outsb`). See docs/VICEROY_NAMES_FROM_MAPEDIT.md §4. |
