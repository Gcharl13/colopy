...
| C4.1 | **F7 Naval: the per-row unit icon is in the wrong place** — found by the census, invisible to every C==JS gate. Same sprite and same colours on both sides; the port draws it at x≈2–7, the original at x≈12–17, once per row at `F7_PITCH 20` (2.5% of the screen). **Not** the nation plate: deleting `rm_nation_plate()` moves the count 1635→1605, so the plate is a rounding error. `draw_f7` right-aligns with `2 + 16 - f.w` and would need `f.w = 6` to land where the original does. Blocked on decoding the shared per-unit info-panel verb `0x181F:0x2BC` (F7 at `@0x039586`) — **not** to be guessed. | `cport/render/colopy_report_render.c` `draw_f7`, `spec/ui/advisor_reports.md:166` |
