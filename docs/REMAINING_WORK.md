...
| C4.29 | **The port's immigration unit count is 130 where the original's is 138.** Found via C4.23: the threshold formula counts "+1 per owned unit", and the original iterates the UnitRecord array (`func_035D9A @0x035DD1–0x035DF8`), which includes Europe-parked ships, their riders, and units aboard on-map ships — all of which the port's `G.units` excludes (they live in `G.europe`/cargo mirrors). On the fixture: 268 vs the stored 284 (both formulas agree at count 138). This paces immigration ~6% fast. Fixing it means counting the Europe-side entities into `immigrationThreshold()` in **both** engines identically (parity-safe) — not done yet because the exact original membership (does a rider count separately from its ship? the 0xEC branch in the accumulator suggests dock-side handling) deserves its own read rather than a plausible sum. | `cport/core/colopy_turn.c` `immigration_threshold`, `port/src/game.js` |
