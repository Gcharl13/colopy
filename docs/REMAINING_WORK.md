...
| C1.14 | **Mine depletion uses a port-runtime bit** (0x80). PARTIAL 2026-08-28: the engine's own silver rule is now in — improve bit **0x04** is the mine/prime bit, and silver on a tile with no detail and no 0x04 yields 1 with a road or an expert, else 0 (`@0x9E41..@0x9EA6`); the depletion ACCRUAL side (`[0xA896]` += per ore/silver worked on detail 6/12, `@0x9E13..@0x9E41`, and the fish accumulator `[0xA895]`) is read but not yet wired to an event. | `port/src/game.js` `fieldYield`, `cport/core/colopy_colony.c` |
