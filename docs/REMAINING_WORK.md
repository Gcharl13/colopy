...
| C4.5 | **The Europe screen renders with the wrong palette** — 7,848 px, **61% of its 20.0% divergence**, and the first fidelity bug the census caught that is a *policy*, not a slip. The port lets `EUROPE.PIK`'s embedded palette win (`rd_pal_fill_placeholders`, mirroring `usePalette` game.js:36 — PIK authoritative, master fills placeholders). The original does the opposite: at all **22 indices where the two disagree**, the DOS capture follows the **master `VICEROY.PAL`**. Byte-checked at 54–59, where PIK says `(113,142,198)…(57,69,150)` and both master and DOS say `(105,138,195)…(40,56,146)`. **Measured, not argued:** substituting master for PIK offline takes the screen 12,817 → 5,448 px. **It does not generalise** — all five report screens show 0 px of palette divergence, so this is Europe-specific and the fix is correspondingly narrow. Flipping it touches both engines' palette policy and every PIK screen, so it needs its own before/after census run. | `cport/render/colopy_render.c` `rd_pal_fill_placeholders`, `port/src/game.js:36` |
...
| C4.4 | **F5 Economic: 398 px at glyph scale** over x 67–245 — reads as text metrics or content rather than a misplaced element. 0.6%. Untriaged. | census `F5` |
