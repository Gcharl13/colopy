...
| C4.1 | **F7 Naval: the per-row unit icon is in the wrong place** — 1,635 px. Same sprite and colours on both sides; the port draws it at x≈2–7, the original at x≈12–17, once per row at `F7_PITCH 20`. **Not** the nation plate as first guessed: deleting `rm_nation_plate()` moves the count 1635→1605, measured. **Advanced 2026-08-20:** the shared verb `0x181F:0x2BC` = `func_00386A` is now decoded far enough to name the cause — the composite is *sprite + a black-outlined PLATE filled with the owner colour + a one-character status letter*, with the plate's position chosen by a unit CLASS (0–4) and its SIZE derived from the letter's width (`strwidth + 3` × `fontheight + 3`). Full write-up in `spec/ui/render_primitives.md` §1b. What still blocks the fix is two sheet-header fields, `es:[bx+0x3E]`/`[bx+0x40]` (`[bp-0x1A]`, `[bp-0x26]`), which carry the sprite's measured size and feed the per-class plate offsets, plus `func_00380C`'s layer semantics. Same blocker as C4.27. | `cport/render/colopy_report_render.c` `draw_f7`, `spec/ui/render_primitives.md` §1b |
...
| C4.2 | **F9 Indian: per-tribe colour divergence** — bands at pitch 21; the port draws green `(117,166,77)` across x 30–82 where the original has gold `(199,162,32)` and black. 5.3%. Untriaged. | census `F9` |
| C4.3 | **F2 Religious: a 37 px block differs** at x 10–46, rows 27–37 — port orange `(178,77,32)`, original black. 0.7%. Untriaged. | census `F2` |
| C4.4 | **F5 Economic: 398 px at glyph scale** over x 67–245 — reads as text metrics or content rather than a misplaced element. 0.6%. **Half of it was C4.9** (the bid/ask straddle — the report prints `market_bid`/`market_ask` and both quotes were one high): 398 → **202 px (0.3%)**, measured. The remainder is still at glyph scale, so it is text metrics or remaining content, not a misplaced element. Still untriaged. | census `F5` |
...
| C4.23 | **F2's missing badge is the crosses gauge — LOCATED.** `func_037958` `@0x0379A7`–`@0x0379E9` reads the player's `PowerRecord +0x2E` (crosses accumulated) and `+0x30` (cross threshold), draws a gauge through `0x181F:0x236` with `ax = 0x39` at **(10, 25)**, and then — gated on `[0x5383] & 0x20` — formats the accumulated/threshold pair and draws it at the same **(10, 25)** via `0x181F:0x13C`. That is exactly the black badge the census found at (10,27)–(46,37) reading "30 ✝". Not yet implemented: the gauge primitive `0x181F:0x236`'s own geometry is unread, so drawing it would mean inventing the bar. | `cport/render/colopy_report_render.c` `draw_f2` |
...
| C4.26 | **Profession byte 0 is read as "none" everywhere except the Europe manifest.** C4.25 proved byte 0 = Expert Farmers from a live frame, but the same `>= 1` guard sits in `isExpert`, `scoutLevel`, `profIs` and the ColonyRecord specialty path in both engines. Relaxing it there changes production and combat numbers, and **nothing available tests those against the original** — the two engines would simply agree on a new answer. Applying it only where a capture proves it, and leaving the rest flagged, is deliberate. A colony-screen census entry is what would settle it. | `cport/core/colopy_colony.c` `is_expert`, `colopy_resolve.c` `prof_is`, `colopy_cmd.c` `scout_level`, `port/src/game.js` `SAV_PROFESSION` |
| C4.27 | **Europe's crossing column: the missing black PLATE.** ~326 px, and now a CONTENT question rather than a placement one. Sweeping the ship anchor over 72–76 against the passenger pitch over 16–18 leaves **(75, 17)** — what the port already has — as the unique minimum, so the geometry is right and the residual is the black plate the original draws behind every figure and every ship. That plate is the per-unit info-panel verb `0x181F:0x2BC` = `func_00386A` — the same verb C4.1 is blocked on. One decode closes both. | census `EUROPE`, `spec/ui/render_primitives.md:47` |
...
| C4.16 | **The Sioux row is inked in palette 12, not its `@TRIBES` colour.** ~126 px of F9's remaining 220. The original draws that tribe's name and level in index **12 (255,0,0)**; `@TRIBES` gives the Sioux **118 (146,0,0)**, and the port draws 118. Every other tribe on the frame matches `@TRIBES` exactly, so this is one tribe on one save and the cause is **unknown**. Not guessed: the obvious candidates were checked and killed — the relation byte's `0x40` bit is set for the Sioux exactly as it is for the tribes that render correctly, and the Sioux carry the *lowest* tension on the save (1) while the Incas carry the highest (100) and render in their own colour. | census `F9` |
...
| C4.18 | **F2 Religious is missing an element, not misplacing one.** 454 px, of which 82 is the DOS mouse pointer (below). The rest is a single black badge at (10,27)–(46,37) carrying the crosses figure and a cross glyph — the original reads "30 ✝" on this save and the port draws nothing there. Supersedes the earlier "a 37 px block differs, port orange vs original black" reading, which described the badge's absence as a colour fault. | census `F2` |
| C4.19 | **Every census capture carries the DOSBox mouse pointer**, ~82 px at (158–172, 98–116) — 168 px on F3, where it lands on busier art. It is a floor no port change can close, so `make census` now REPORTS it as its own column rather than subtracting it: a number this tool quietly edited would be worth less than one it explains. The fix is to park the pointer off-screen before `shot()` on the next `--capture` run, which will move every baseline frame and every figure quoted against them. | `tools/screen_census.py` |
