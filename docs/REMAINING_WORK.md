...
| C3.11 | **NEW 2026-09-02** — the engine's click path opens the outside-jobs menu for a selected GARRISON figure too (no job test @0x02A07E), and the op then runs `classify_pair_bounds` mode 1 = **re-equip the unit in place** (`func_009318` @0x009576..@0x0095C0: type := `byte[0x2F5+job]`, orders 0, Pioneer tools) — arming/outfitting a unit inside a colony. The ports' garrison selection opens @UNITOPTIONS and never reaches it. | `port/src/game.js colonyPopupCommit`, `cport/game/colopy_input.c` |
