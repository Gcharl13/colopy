...
| B3.11 | **The colony BUILD view's BUY button diverges, one bug on each side.** With a target set and the completion cost affordable, the C rush-buys outright — gold spent, target cleared, no confirmation — while the JS does nothing at all: no `@BUYME1` ask, no `@BUYME0` refusal, no spend, even though `hit(224,142, BUILD_BTN.buy)` and `G.colonyView === VIEW_BUILD` both evaluate true at that instant, so `rushBuy()` should be reached. Found 2026-08-17, the first thing the newly-live colony pointer coverage turned up. Reproduction is the disabled `C(224, 142)` line in `colony_clicks`. | `port/src/game.js:3343` `rushBuy`, `tools/input_compare.py` `colony_clicks` | — |
