...
| Europe (§26.9) | Recruit / Train | **DONE** 2026-08-04 | Recruit reads the three dock slots against `@CLASS.transport_cost` (300–2000) with the §17.6 candidate ladder refilling each slot; Train lists the 17 `@JOB` rows whose `europe_value` is not −1. |
| Europe (§26.9) | Purchase page | **PARTIAL** | Implements the three goods purchases §9.4 names explicitly (Muskets ×50, Horses, Tools ×100) at the ask. Ship and Artillery rows are **omitted**: no price table exists in the shipped data, and Artillery is only specified as "base + artillery_bought·100" with no base. |
