...
| Native settlement ownership patch | **TBD** | The port fills a 6×5 tribe-colour rect. **Port-invented art with no engine equivalent** — native marker frames 10–13 carry no baked pennant at any offset, and nothing in the village painter draws an ownership patch. The engine's real per-village overlays are the alarm marks and the mission cross. |
| Mission cross | **PARTIAL** | Shape and Y offsets byte-read off the village painter: backing `(XB, py+5, 5, 6)` @`0x0041D7`, vertical `(XB+2, py+6, 1, 4)` @`0x004203`, horizontal `(XB+1, py+7, 3, 1)` @`0x004222`. **XB is TBD** — the engine's base is `px+6` (@`0x00407D` is `mov ax,[bp-0x64]; ADD AX,6`) on the path skipping the alarm-mark loop and `px+8+2·marks` through it; the port mirrors the no-marks path, which is its own choice. The DGROUP `0x848` colour table is undumped, so the expert-brightness rule (table[power] **minus 8** when the mission byte's `0x10` bit is clear, @`0x0041C6-0x0041D4`) is unreproduced. |
...
| Drag and drop | **PARTIAL** | Implemented against `func_00D106` @`0x0D106-0x0D1C9` (five per-poll booleans; **no** pixel threshold, @`0xD16F`), the payload word `[0x8D54]`/`[0x9E3A]` and its detail fields, the byte-exact region tables (`func_0299A0`, `func_03200A`) tested in the engine's own order, and the drop-legality tables (`func_02BB8A` @`0x2BBBD-0x2BBF9`, `func_0353DE` @`0x35416-0x35464`). **Open:** the `0xC0C:6` tick rate, so the 8-tick hold deadline cannot be converted to ms; the drag-cursor hotspot (`func_00DB80`); the unit ghost frame; and colony goods, whose only targets are {5, 8} where region 8 is a ships-in-port dock the port does not draw. **No map-view drag** — no reference to `[0x7E4..0x7FA]` anywhere in `0x63000–0x68000`. |
