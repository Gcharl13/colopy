...
| Live Among / Speak With Chief / Incite / Demand Tribute / Attack Village | **TBD** | The rows are offered per the shipped gating and report themselves as absent. Their gates read the tribe-record **posture** byte `+0x5236`, which is traced but not decoded, so those three gates are currently unconditional. |
