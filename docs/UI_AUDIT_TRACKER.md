...
| 12 | Customize (CUSTOMIZ.PIK) | **func_070060** (loads CUSTOMIZ.PIK push 0x2022 @0x070088) | docs/FRONTEND_SCREENS_VICEROY_DECODE.md §5 | DONE (B: PIK load; draw-all func_06FF94; 4×3 grid col·76+10/row·60+16, cells **72 wide × 48 tall** @0x06FDF0 (corrected 2026-09-09 — fill bx=0x48/push 0x30 @0x06FE61, box (x,y)–(x+0x47,y+0x2F) @0x06FEB1, hit (x,y,0x48,0x30) @0x07020B; the PIK's frames measure 72×48); title y4 ink0xFD/bg0xFE; cell ink0x0A/0x0E; sel=[0x1E7E]×4 default[1,1,1,1]; keys @0x070114..0x0701DD Up/Down = value (v+2)%3/(v+1)%3, Left/Right (and Backspace/Tab) = axis, Space = value+1, Enter finish, Esc back; finish=click&mouseY≥185 @0x07027E; captions = @MISC 144..161 (18-string run vs the 18-word BSS table — consistent, loader untraced); **PORTED both engines 2026-09-09** (RULINGS 2026-09-09c; render compare `customize 0/2`) — TBD: the 0x181F:0xE2 band/frame sprites (sheet [0x2DA8]), 0xCE fill-vs-outline, Diff/Power sub-popups func_070302/070494 |
| 13 | King audience / loss / win (KING*) | renderer func_075352 @0x075352; boot caller func_075594 @0x075594 (from func_0755CC @0x0756DC); trigger func_02F3A2 @0x02F3A2 | docs/ENDGAME_SCREENS_VICEROY_DECODE.md §1; spec/ui/woodcuts_and_intro.md §2 amendment 2026-09-02 | DONE (KING1/KINGLOSE/KINGWIN select @0x075430 + KINGLSS1.PIK @0x07536E + nation banner ENGLND1/FRANCE1/SPAIN1/DUTCH1 @0x0753B8 + FONTKING @0x0754F2 byte-cited; **the "pen (242,47)" was a misreading — `[0x1F4A]=0xF2/[0x1F50]=0x2F/[0x1F52]=0` @0x075526..0x075532 are the popup INK SLOTS** (RULINGS 2026-09-02f); the scroll is laid out by the popup engine @0x075540 from `@VICEROY`'s own `@width=78 @x=232 @y=21`; boot variant queues tune 0x3E @0x07544B; no caption; sheet placement by frame anchors (RULINGS 2026-07-31). TBD: which glyph level maps to which ink slot; the endgame body text runtime); **E4/E5 2026-09-02 (screens track, RULINGS 2026-09-02y):** **placement byte-cited 2026-09-02:** banner + king sheet blitted into the PIK buffer with the anchored verb 0x181F:0x2F8 at their own descriptors (@0x7541A..0x7542B, @0x75477..0x7549D) → KINGLOSE (0,20), KINGWIN (27,1), KING1 (0,12), <NATION>1 (32/30/35/34,0), <NATION>2 (52/49/59/55,0); text laid out by the key's directives (@KINGLOSE 232/31/w68, @KINGWIN 202/125/w90); triggers @0x2F542..0x2F55F (win) / @0x2F670..0x2F6B0 (lose). Ported both engines (REMAINING_WORK E4/E5). Residual: text RGB (runtime, §3c A); KING2/WIN/WIN-FWRK orphans |
...
| 15 | Score screen (SCORE01–24) / F10 | func_03A9C0 @0x03A9C0 | docs/ADVISOR_REPORTS_VICEROY_DECODE.md §F10 (+ spec/ui/advisor_reports.md, spec/ui/cinematics.md §4 amendment) | DONE — band selector byte-cited: i=1..0x18, panel=i-1 for every i with **i·i/3 < scaled (UN-halved mult·base/100; `jge` skips @0x3AA58)**, rating halved AFTER @0x3AA6A, clamp ≤0x17 @0x3AA71, no screen for base≤0/panel<0/display=0/[0x5382]&0x10; SCORE<panel+1>.SS (str 0x11CF +`0x182`@0x3AADA) over WOODPAN2 (str 0x11D7 @0x3AAFF) **through the plate's palette** ([0x23F2:0x23F4] @0x3AB46..0x3AB84); **layout byte-cited 2026-09-02:** @EXPLOITS ×3 centred x0 w320 y=5/12/19 colour 0xFC (@0x3ABC7..0x3AC0B), @SCORE rows 0..panel at y=0xC3−7(i+1) centred x0xA0 w0xA0 0xFE/0xFC (@0x3AC1A..0x3ACA8), caption (field 2, %STRING0 = strrchr(name,' ')) centred x0x22 w0x8C y0x8E (@0x3ACB2..0x3AD0B), plate anchored @0x3AD4C, tune @0x3AD51..0x3AD6D, key wait @0x3AD86; FONTTINY only on this page (FONTINTR `[0x268A]` @0x3B054 is F10's). Ported both engines (REMAINING_WORK E3). Residual: live figures (runtime), the 0x5426 string's identity (read as the country) |
| 16 | Declaration of Independence (DECOIND) | **func_03DA2A** @0x03DA2A (~~dispatch slot 4 @0x3EA0B~~ **no static caller — 2026-09-02:** @0x3EA0B is `ljmp 0x191F:0x364` = `func_03C638`, and no `lcall`/`ljmp`/far pointer to `0x191F:0x109A` exists; TBD live capture) | spec/ui/declaration_independence.md (+§7 amendment) + spec/ui/cinematics.md §5 | DONE — in VICEROY.EXE: loads **DECOIND.PIK** (DECLARAT.PIK is an orphan, B-negative) + per-letter **DEC-UPPA..Z / DEC-LOWA..Z / DEC-SQIG** sprite sheets; sprite-based typewriter reveal start x=126 y=148 (@0x3DC3C/@0x3DC42), per-class x/y advance (@0x3DC58..0x3DCFD, width @0x3DD16), frames = descriptors 1..n (@0x3DD30), wrap x≥220 (@0x3DE04), final key-wait @0x3DE17. **Cadence RESOLVED 2026-09-02:** one stroke per 60.8766 Hz tick (`0xC0C:6` → `[0x267A]=0x92E8` @0xC857; ≥5 ISR-tick floor @0x3DDB9). Ported both engines (REMAINING_WORK E2). TBD = the caller only |
...
- **SCORE (F10) = `func_03A9C0`**: band = `clamp(maxᵢ{i: i²/3 < lvl·score/100}, 0..23)` → SCORE01..24
  ...
  7..22→0x25, 23→0x24).
- **KING = `func_075352`** (FONTKING; ink slots 0xF2/0x2F/0 — the old "pen 242,47" was the ink-slot stores, corrected 2026-09-02; KING1/KINGLOSE/KINGWIN by outcome; sheets placed by their frame anchors).
...
- ~~**Tutorial overlays**~~ **DONE** → `spec/ui/tutorial.md`: dispatcher func_020F50 (10 unit-focus tutorials, full trigger conditions) + 9 event-driven sites; T18 ungated (original quirk). **2026-09-02** (`spec/systems/tutorial.md` amendment): the gate at every site is the Tutorial-Hints option `[0x5382]&0x80` (on iff Discoverer at new game, @0x074348); T16/T17 once-flags `[0x5380]` 0x10/0x20 (@0x0286FF/@0x035C2B); T2 has no once-flag. Ported to both engines (B4.2 closed).
...
- ~~**Intro caption cards**~~ **DONE** → `spec/ui/woodcuts_and_intro.md` §2 (+ amendment 2026-09-02): LEVN PIK slideshow over world generation, one card per 0x23A ticks = 1873 ms (tick unit closed), popup-engine layout from `@BUILDn`'s `@width=310 @y=30` with ink slots 0x0E/0x36 (the old "pen (14,54)" was the ink-slot stores), non-modal flag 0x20, per-card substitutions (card 7's %STRING0 by slot persistence), a key/click = the done flag, Alt-X/Alt-Q quit. TBD: the rendered text y/pitch (no DOS card capture to diff against).
...
| REPORTS | **DONE (layout)** | **2026-09-02 census re-capture (C4.19, pointer parked): F2 0 px, F5 15, F7 0, F9 12 against the DOS frames; F3 12,162 (declared).**  All nine adviser reports plus the F1 pedia route are built from real state over their own **REPORT&lt;N&gt;.PIK**, with **no adviser portrait** — the painted scene in the plate is the adviser. Every body layout is now measured against a live DOSBox frame, not inferred: F2/F3/F4/F6/F8/F10 on 2026-08-05 and F5/F7/F9 on 2026-08-06 (`docs/screens/live_2026-08-05/`, plus a seven-tribe/seven-ship/full-market set from the shipped 1653 save in `docs/screens/live_1653_save/`). Whole-frame port-vs-live diffs are down to 75 px (F7) and 299 px (F5), all of it live game data. **2026-09-02 (C4.17):** F9 MISSIONS cell implemented in both engines from `func_03744A` @0x037638–@0x03777B (strings @MISC 27/28 via the LABELS loader), synthetic-mission parity scene added. **Open:** ICONS 113..117 native-portrait selection (three frames observed, no rule — the port draws 116); F9 pagination `func_039E98` not wired; ~~F5's second view (@MISC 91/92 upkeep)~~ RESOLVED 2026-08-29: @MISC 91/92 have no consumer in the EXE (B3.2) — F5 has exactly one view; F3's per-section y advance. See RULINGS.md 2026-08-06. |
...
| Abandon / rename | **DONE** | `@ABANDON`/`@ABANDON2` are the LAST-COLONIST-OUT prompt (validator `func_025A1E` code 3 → handler @0x0288C8..@0x02892A; "2" when owner colonies < 2 AND year > 1575; row 1 proceeds) — there is no abandon command or key in the EXE (corrected 2026-09-02, RULINGS 2026-09-02l; the port's shift-A was an invention and is removed). `@RENAMECOLONY` (R) unchanged. |
...
| Sound Options | **DONE** | Three rows. Since 2026-08-17 the cport audio milestone consumes them: `cport/audio/` ports the switch semantics (bit1/bit2/bit3 of `[0x5386]`, pinned @0x023301) and gates playback on them. Since 2026-09-02 leaving the dialog reaches the engine (`SND_SWITCHES`: the mirror + the stop @0x2333B when any switch is off), the default word is the EXE's 0x0E (@0x755EB), and the input oracle drives the dialog (slice 9). The JS build itself still has no audio. |
| Which options act | **PARTIAL** | **Combat Analysis** and, since 2026-09-02, **Tutorial Hints** (0x0080 — the byte gate of every @TUTORIALn lesson, RULINGS 2026-09-02g; on iff Discoverer at new game, @0x074348) change behaviour; the rest are stored and displayed truthfully but unconsumed (there is no piece-slide animation, no autosave, no water cycling in the port). |
