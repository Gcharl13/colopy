...
void score_parts(score_parts_t *s) {
    ...
    for (int ci = 0; ci < CS.n_colonies; ci++) {
        ...
        for (int k = 0; k < c->population && k < 32; k++) {
             ...
             * (RULINGS 2026-09-02x: sav1653 scored 299 vs the JS 315). */
...
void end_game_sequence(void) {
    ...
    /* the page's tune (func_03A9C0 @0x3AD51..0x3AD6D — inside the page
     * path, after the no-screen gates @0x3AA88..0x3AAA0): 0x24 on the
     * top band (>= 0x17), 0x25 above band 6, else 0x21 */
    if (CR.score_show >= 0)
        snd_play(CR.score_show >= 0x17 ? 0x24
                 : CR.score_show > 6 ? 0x25 : 0x21);
