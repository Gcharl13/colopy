...
**Phase 6 delivered that container**: `tools/gen_sd_pack.py` builds
`cport/pak/COLOPY.PAK` (git-ignored, ~3.0 MB) — 67 sprite sheets (8-bit
indexed + frame tables), 28 PIK backgrounds, the 5 engine fonts verbatim,
VICEROY.PAL as RGB888, the CYCLE.DAT band, and a `TEXT` section carrying
the exact `colopy_text.c` content (3,137 key/value pairs) so a
flash-constrained build can drop that object and read the strings from SD.
TOC contract: generated `cport/data/colopy_pak.h`; validator + census:
`cport/host/smoke --pak` diffed against the JS DATA census by
