...
/* AUDIO (docs/AUDIO_PORT.md): the cport/audio engine + the on-board
 * speaker path (I2S BCLK 22 / LRCLK 21 / SDATA 23, amp gate GPIO 30 —
 * Elecrow Lesson12) is ON by default (the shell's own fallback is
 * `#define COLOPY_AUDIO 1`); it needs COLAUDIO.PAK on the SD card
 * (python3 tools/gen_audio_pack.py after the tools/audio capture
 * pipeline) and falls back to COLDIG.BIN direct without it.  Uncomment
 * the line below to build a MUTE sketch.  The macro is tested with
 * `#if COLOPY_AUDIO`, so it must carry a 0/1 value — an empty
 * `#define COLOPY_AUDIO` is a compile error, not "on". */
/* #define COLOPY_AUDIO 0 */
...
/* shell-side cue edges (the core/game layers stay untouched): the title
 * screen = the composer's raw tune 0x33 @0x75C2A; the King's audience
 * queues tune 0x3E (func_075352 @0x07544B, the boot variant); the cards
 * start tune 0x39 with 0x25 queued behind it (func_0755CC @0x0756E4 /
 * @0x0759A0 = au_on_new_game); woodcut = a new plate on screen.  (Until
 * 2026-09-02 the new-game cue hung on a BRIEFING->MAP edge the chain no
 * longer takes, so the board never played it.) */
