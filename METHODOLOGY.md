...
4. **Data-first / start from the basis.** Specs are *derived from* the extracted
   **basis**, never from imagination. Before describing a system, read the actual
   game data for it. An empty or missing key is **never** interpreted by guessing
   — pull the real body from the `.TXT`, or mark `TBD`. (Two systems were once
   invented from *empty* keys because the old extraction dropped the bodies.)

### The basis (the primary artifacts every spec rests on)
- **Text** — `data_extracted/text/*_sections.json` (flat `@KEY`→body) and
  `*.full.json` (legend + directives + body + CSV rows). Regenerated by
  `tools/extract_txt_sections.py` from the real `.TXT` (`raw/COLONIZE/`, staged
  by `bin/reconstitute.py` + `col.zip`). **Complete bodies** — fixes the old
  lossy dump that scrambled directives (`@width=`/`@default=`) as keys and
  emptied bodies.
- **Tables** — `data_extracted/tables/*.json` (`tools/build_tables.py`): the
  NAMES/TRIBE CSV sections as named, byte-anchored tables. Values are verbatim
  game-data bytes; column names are the file's own legend; byte_anchors cite the
  EXE loader/use-site where known.
- **Binary strings & variables** — `data_extracted/viceroy_strings.txt`
...
0. Read the **basis** for the topic first: its `@`-section body/columns
   (`data_extracted/text` + `…/tables`) and any record offsets (`docs/DATA_MODEL.md`).
