...
## Pipeline
    ...
    python3 map_coldig.py                    # -> data_extracted/data/coldig_slices.json
    python3 trim_masters.py                  # -> captures/masters/ + manifest
    python3 ../gen_audio_pack.py             # -> cport/pak/COLAUDIO.PAK + header

`captures/` is git-ignored (regenerable); the two committed artifacts are
`data_extracted/data/coldig_slices.json` (the empirical SFX id→slice map)
and this directory's manifests, which make the regeneration auditable.
