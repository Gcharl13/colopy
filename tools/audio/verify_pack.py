...
- codec 0 (PCM slice): sha256 vs the COLDIG.BIN slice named in the
  byte-decoded index data_extracted/coldig_index.json (bit-clean or fail).
  That index superseded the empirical data/coldig_slices.json on
  2026-08-17 — see tools/gen_audio_pack.py and formats/BIN.md.
...
def main():
    ...
    samples, sfx_map = idx["samples"], idx["sfx_id_to_index"]
    ...
    for i in range(count):
        ...
        if codec == 0:
            ent = samples[sfx_map[f"0x{sid:X}"]]
