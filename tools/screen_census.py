...
REPORTS = {
    ...
    "EUROPE": ("EUROPE", ["--rendereurope", FIXTURE, str(PAK), "{out}", "0", "0", "0", "0"],
           "OPEN (4.8%), down from 20.0%. Five fixes, each measured:\n"
           ...
           "dock (colopy_europe.c euro_sentinel / game.js euroSentinel)\n"
           "         4,116 -> 3,396  C4.8 the six cargo slots are drawn "
           "UNCONDITIONALLY (func_0314DC @0x0314F1, grid func_0314AE "
           "@0x0314AE); the port hung them off `if (ship)`, so an empty "
           "harbour showed bare backdrop. ICONS 122 matches the original's "
           "empty slot pixel-for-pixel\n"
           "         3,396 -> 3,140  C4.9 the bid/ask STRADDLE the record's "
           "price level: bid = max(0, level-1) func_030590 @0x030590, "
           "ask = max(0, level+spread) func_030566 @0x030566. The port "
           "quoted the level as the bid and burden+1 as the ask, so all "
           "sixteen market-bar prices read one high\n"
           "         3,140 -> 3,102  capture-state alignment, not a fix: DOS "
           "enters with market cell 0 selected, so the census now renders "
           "with market_sel = 0 instead of none\n"
           "        What is LEFT, banded: the top bar rows 0-6 (578 px, x "
           "50-269), the recruit buttons rows 88-97 (142 px, x 281-317), the "
           "crossing panels + recruit rows 99-132 (933 px), the Bound-For "
           "column rows 140-162 (406 px, x 73-138) and the market-bar ICON "
           "row 178-192 (1,011 px). The price row 193-199 is now down to 32 "
           "px. The icon row is a PLACEMENT question and the measurement "
           "says so: shifting the port's icons one pixel left drops that "
           "band 1,049 -> 684 px, so a whole-row -1 is most of it and "
           "something per-sprite is the rest. Not fixed here, because the "
           ...
           "y = 0x84 (132) at step 4 beyond -- and the ship and its "
           "passengers share one ordinal sequence. The port switches band "
