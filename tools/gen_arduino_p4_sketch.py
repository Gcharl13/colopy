...
4. microSD (FAT32, files in the card ROOT): copy `cport/pak/COLOPY.PAK`
   ...
   `raw/COLONIZE/COLONY00.SAV`.  (Saves are plain DOS `.SAV` files —
   unit build targets and trade routes ride in them exactly as the
   original writes them, so a save moves between this board and DOS.)
...
# arduino-cli build profile (2026-09-08) -- the same Tools-menu choices the
# README's step 3 spells out for the IDE, as a `sketch.yaml` so
# `arduino-cli compile --profile colopy-p4` builds without hand-clicking
# them.  Library paths are the vendored Elecrow bundle, relative to the
# sketch folder.  The core version is the pin carried over from the
# sibling port's profile; this tree has no arduino-cli run of its own on
# record, so treat the profile as the IDE steps written down, not as a
# reproduced build (the IDE path IS verified: user builds 2026-08-16 on).
SKETCH_YAML = """\
# Generated by tools/gen_arduino_p4_sketch.py -- edit SKETCH_YAML there.
# Mirrors README.md step 3 (the Arduino IDE Tools menu) for arduino-cli:
#   arduino-cli compile --profile colopy-p4 .
#   arduino-cli upload  --profile colopy-p4 -p <port> .
# PartitionScheme=huge_app: the 3 MB app slot; the sketch is ~1.03 MB
# (2026-09-03 IDE build), so no custom partition table is needed.
profiles:
  colopy-p4:
    notes: "CrowPanel Advance 7in ESP32-P4; IDE settings per README step 3"
    fqbn: "esp32:esp32:esp32p4:ChipVariant=prev3,PSRAM=enabled,USBMode=default,CDCOnBoot=cdc,PartitionScheme=huge_app,FlashMode=qio,FlashSize=16M,UploadSpeed=921600"
    platforms:
      - platform: "esp32:esp32 (3.3.11)"
        platform_index_url: "https://espressif.github.io/arduino-esp32/package_esp32_index.json"
    libraries:
      - dir: "../../p4/arduino_libraries/ESP32_Display_Panel"
      - dir: "../../p4/arduino_libraries/ESP32_IO_Expander"
      - dir: "../../p4/arduino_libraries/esp-lib-utils"
default_profile: colopy-p4
default_port_config:
  baudrate: 115200
"""
...
def main():
    ...
    (DEST / "sketch.yaml").write_text(SKETCH_YAML)
    print(f"wrote {len(files) + len(REF_CONFIGS) + 2} sources + README + "
          f"sketch.yaml to {DEST.relative_to(ROOT)}")
