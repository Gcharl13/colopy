...
**Overall confidence:** NativeSettlement base/stride + a few offsets `BYTE_VERIFIED`; attitude/trade/raid logic `RECONSTRUCTED`/`TBD`. **Canonical primary:** `docs/DATA_MODEL.md` NativeSettlement; `data_extracted/text/NAMES_sections.json` `@TRIBES`/`@ATTITUDE`/`@ATTITUDINAL`/`@LEVELS`/`@MISSION`; `data_extracted/text/GAME_sections.json` native keys.
...
- Attitude escalation/decay, trade pricing, tribute amounts, mission conversion: **TBD** (`func_03ECF0` adjacency is the per-unit confrontation AI per RULINGS — not the price math; do not assert).
