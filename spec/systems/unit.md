...
| `0x20` | `OR 0x20` @0x04CE44 (`func_04CC50`, type 0x0E Merchantman) | `AND 0xD1` @0x04CEB1 | `@0x04F3D7`, `@0x0507A3`, `@0x052467` | **per-site BYTE_VERIFIED = "this unit is a Merchantman" trade-AI exclusion marker.** SET `@0x04CE3D..0x04CE44`: `cmp [bx+0x3146],0xe / jne / OR 0x20` (+`[bp-0xa]=1`) — set iff unit_type==0x0E (Merchantman). CLEAR `AND 0xD1` @0x04CEB1 (re-derives the 0x20/0x08/0x04/0x02 scratch nibble each pass). TEST `@0x04F3D7`: `test ...,0x20 / je / jmp skip` — when set, the unit is *skipped* by that trade-AI branch. So 0x20 is a per-pass Merchantman tag, NOT a persistent attribute. Exact one-word label still TBD (overloaded with the same recomputed-nibble role). |
