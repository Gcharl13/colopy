...
**Overall confidence:** event text + **effect mechanics `BYTE_VERIFIED`** (`func_03C638`: power selection + map-tile/unit/colony ownership transfer + power-elimination); **trigger LOCATED** (`@0x02393A` end-game dispatcher, gated `[0x53D0]`/`[0x53D2]`; `tools/find_callers.py`); **event-id IDENTIFIED = `0x68`**, dispatched by the master event-id switch `func_0235D6` — `mov ax,[bp+6]` (event id) `@0x0235E2`, big-switch path `@0x023DC8` (`cmp ax,0x300`/`sub ax,0x1b`/`cmp ax,0x5c`/`jbe`/`shl ax,1` `@0x023DD5`), jump-table `jmp word cs:[bx+0x2f08]` `@0x023DE3` whose entry idx `0x4d` (id `0x1b+0x4d=0x68`) targets the succession gate `@0x02391C` → `lcall 0x191f:0x364`→`func_03C638` (func_0235D6 @0x0235E2; func_03C638 @0x03C638). **Jump-table entry byte-decoded 2026-06-27:** table base `cs:0x2f08` lands at file `0x23DE8` (codebase ip0 = file `0x20EE0`); the word at file `0x20EE0+0x2f08+(0x4D*2)=0x23E82` reads `0x2A3C`, i.e. ip `0x2A3C` → file `0x2391C` — the succession gate, confirming the id `0x68` routing arithmetically (cross-check: id `0x70` entry `0x2A24`→file `0x23904`). The succession gate body is purely state-driven: `cmp [0x53D0],0x4B` `@0x2391C`, `test [0x5381],0x80` `@0x023923`, `cmp [0x53D2],0` `@0x023930`, `lcall 0x191f:0x364` `@0x02393A` — **no RNG/odds read inside the dispatcher**. The remaining **firing-probability/odds gate** that *enqueues* event id `0x68` into this dispatcher is **runtime-scheduler state, not statically resolvable** here: `func_0235D6` is a bare `enter 0x1e`/`retf` overlay entry (page 1, seg `0x2F66`) with **no caller in `functions.jsonl`, no entry in `thunk_resolve.json`, and no function-pointer to its address (`2F66:26F6`) in any data table**; the two `push 0x68` sites in the corpus (page_02 `@0x02652C`, a draw-coordinate arg to `lcall 0x181f:0xce`; page_1A `@0x0729AB`, a UI string-index in the consecutive `0x67/0x68/0x69/0x6a` sequence to `lcall 0x191f:0x91c`) are both unrelated to the event queue. **Closing this leaf requires a runtime capture** of the per-turn event-queue scheduler (the overlay loader/scheduler that pushes the event id onto `func_0235D6`'s stack); the static substrate's overlay call-graph for this entry is unresolved. · **Canonical primary:** `data_extracted/text/GAME_sections.json`
