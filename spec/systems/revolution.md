...
- "No wars during revolution" rule (`@NOWARSDURINGREV` = "Foreign colonies cannot be attacked during the {War of Independence}."). **String present (B)** but the **enforcement/emit site is byte-confirmed ABSENT from the available disasm**: `code/VICEROY/strings.json` records it (file `0x1F493`) with `xrefs: []`, `viceroy_source/docs/event_catalog.json` lists it `func: null / reachable: null` — uniquely among the WoI-rule strings, whose siblings DO resolve (`@NOCOLONIESEITHER`→`func_022542`, `@NOMAYORSDURINGREV`→`func_059B90`), and no `PUSH 0x0b36` handle-push exists in `VICEROY.EXE` (count 0). The WoI-rule gate PATTERN is byte-verified on the sibling: `func_022542 @0x22576` does `test [0x5382],1; je` (WoI-declared bit 0) → emit `@NOCOLONIESEITHER`. The specific `@NOWARSDURINGREV` enforcement site is **TBD-HARD** (emitter stripped/overlay-unreachable; no readable anchor).
