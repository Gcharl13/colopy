...
| 6 | Immigration / cross rate | crosses loop `func_0363A2`; threshold/production `func_035D9A` | `systems/immigration.md` | **RESOLVED 2026-06-19:** crosses loop + threshold shape **B**; **per-turn cross increment B** (base 2 + per-colony `+0x05`, spawn when `+0x2E>+0x30`); dock pool `+0x02..+0x04` confirmed (Brewster). **RESOLVED 2026-06-28:** per-slot immigrant-**type** selector = `func_034C24`, a 3-tier difficulty-weighted RNG ladder (slot=`random_int(0,2)` `@0x36462`; `thr=(lvl+3)>>1`; tier1 `rand(1,15)`→class 0x1A/0x1C, tier2 `rand(1,10)`→0x19/0x1C, tier3 `rand(1,8)`→0x1C; Brewster FF bit 0x14 upgrades to Elite 0x1C; refill when `(turn&3)==1`). **B.** |
...
| 9 | Map generation | ✅ decoded (post-gen pass order + customize params, B) | `systems/map_generation.md` | **RESOLVED 2026-06-28:** landmass = **iterative random blob-stamping** (`func_064A10`: seed `random_int(1,0x7fff)→[0x190]`; ocean-fill Ocean=25; grow loop stamps inward-biased random blobs via `func_0643F8` until land count `[0x2d22] ≥ (p1+p2+1)·0x140`; then region/smoothing passes). **B.** |
| 10 | Event triggers & timing | Lost-City `func_061454`; raid `func_05BE84`; trigger = features `0xB0` | `systems/events.md` | **RESOLVED 2026-06-19:** Lost-City trigger (features-layer `0xB0`, runtime-verified) + all 9 `@LOSTCITY<n>` outcomes + FoY=8 immigrants **B**; raid outcome→key wiring **B**. **Magnitudes RESOLVED 2026-06-20:** n=3 gold `10·3d8`, n=7 gift `2·4d10`, n=2 Cibola treasure `100·(10·(scout+2)+1d20)` (`func_061454 @0x6166A/@0x61776/@0x617C6`), scout/difficulty-scaled. **RESOLVED 2026-06-28:** burial rolls byte-verified — `@BURIAL2` gold `10·3d8` `@0x61A1D`, `@BURIAL3` treasure `[bp-0x32]=2·(1d8+2·(scout+5))` shown ×100 `@0x61AF2`; n=2 Cibola treasure ×100 `@0x61AF2`. **B** (`events.md` §2). |
| 11 | Save / load codec | SAVE orchestrator `func_072F7A` → serializer **`func_0734F8`** (B); loader `func_073BB0`; HALLFAME.DAT `func_03ADA6` (B) | `systems/save.md` | **SAV format now B** (2026-06-19, verified vs EXE): magic `"COLONIZE"`+`0x1A`, file `COLONY<slot>.SAV`, then 4 tables at full stride (Colony·0xCA / Unit·0x1C / Power 4·0x13C / Native·0x12). HALLFAME.DAT B. **RESOLVED:** PowerRecord on-disk field order = the runtime struct order, **no reordering** (`save.md` §3, line ~112). **B.** |
| 12 | Scoring weights | scaler **`func_03A9C0`** (B); component sum `func_039EE2` (resolved) | `systems/scoring.md` | **Scaling + population component B** (2026-06-19): difficulty mult `[4,5,6,8,10]`, `score=(mult*base)/100>>1`, rank, accumulator `[0x372]`; population gates `{0x19,0x1A,0x1B}→+1`/`0x1C→+2`/else `+4`. **RESOLVED 2026-06-28 — all 7 component weights B** (`func_039EE2`): population, **father +5 each**, sentiment `[0x53D0]×1`, **razed `×−(1+diff)`**, gold `min(gold/100,100)`, bells `bells32/1000`, revolution `(1780−turn)·2`; sum × `((8>>n)+8)/8` difficulty mult (`scoring.md`). |
