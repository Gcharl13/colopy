...
| 2 | Market price drift | accumulator **`func_0305A8`** (B, in `viceroy_source/src/market/pricing.c`); `PowerRecord +0x4C[16]`; `0x53EA` | `systems/market.md` §3 | Commodity set B; per-commodity accumulator B. **Note:** the old `0x181F:0x9A4` attribution was WRONG — that thunk is a shared utility (92 callers across combat/native/colony), not the drift fn (see `tools/rtlink/THUNK_FOLLOWING.md`). Remaining: the per-turn driver that calls the accumulator. |
