...
| 4 | Tax pretext selection | `func_036138` (pretext builder); clamp `func_034318`; gate `func_0349F4` | `systems/king.md` §3 | **RESOLVED 2026-06-19:** pretext chosen by escalating gate on `[bp-0x52]` (thresholds `0x28A`/`0x3B6`/`0x44C`) → KINGWIFE→KINGWAR→KINGNAVACT→KINGSTAMPACT (`@0x362C7..0x36371`); clamp **75** (`0x03434F`); 60 gate (`func_0349F4`). All **B**. Remaining: the `[bp-0x52]` setter. |
...
| 7 | Diplomacy outcomes | **handler `func_057F4E`** (B, page 0x0F); war-matrix `0x883C` via `[bx+si-0x77C4]` | `systems/diplomacy.md` | **LOCATED 2026-06-19:** the "no xrefs" was a grep error (literal vs `-0x77C4` displacement). `func_057F4E` (meeting/parley) byte-verified vs EXE: war bit `0x02` set `@0x58A7B`, peace bit `0x80` clear `@0x58BE1`, treaty cooldown `[0x53C8+pw*2]=turn+0x10`. Remaining: per-pair bit layout + AI willingness thresholds. |
