...
corrections to §5 recorded in `notes/rulings/RULINGS.md` 2026-09-02h.
...
- **The scheduler's PRNG is the game's.** `0x9EF:0x32` = `func_00C322` =
  ...
  call in the game uses. Port decision: RULINGS 2026-09-02h (private state,
...
| 6 | `0x2C660` | colony screen `func_02C5D4`: the second draw pass runs only when `[0x34A] ≥ 0` (`jl 0x2c66f` @0x2C640); `[0x34A]` is written @0x2D2F7 by the colony-report helper after the `BUILT` (0xD6F) report when its answer `[bp-2] == 1` and the building `[bp-8] ∉ {0x10, 0x1F}`, and reset to −1 @0x2EAB2 | `0x54` | **DONE 2026-09-03** — both engines carry the arm and the cue (RULINGS 2026-09-03i3): a completed BUILDING sets the slot to its index unless it is 0x10 or 0x1F, entering the colony screen sounds 0x54, and leaving resets the slot to -1. FLAGGED: `[bp-2]` (set above the function's window) is read as "a building completed in the human player's colony"; the two-frame zoom HIGHLIGHT is not drawn. No fixture arms it, so the cue is unexercised by the oracles. |
...
| 36 | `0x5D600` | the capture/destroy path @0x5D5D0 (`0x7E0` → `call 0x5e72d`), show | `0x4A` | — (the port's capture takes #35's 0x4B; see RULINGS 2026-09-02i) |
