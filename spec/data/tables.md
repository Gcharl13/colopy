...
_Generated by `tools/build_spec_data.py` from `data_extracted/tables/*.json`. Values are VERBATIM game-data bytes; columns are the files' own `;` legends; `byte anchor` cites the EXE loader/use-site or `TBD`. `id` columns: the row index IS the runtime id (CLAUDE.md hard rule)._
