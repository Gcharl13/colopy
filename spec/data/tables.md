...
> byte anchor: consumed by colony build/production; loader TBD; legend documents cols [B legend]
...
> byte anchor: profession; UnitRecord +0x15 = class/profession [B]; europe_cost -1 = not purchasable

| name | expert_name | student_level | europe_cost |
|---|---|---|---|
...
> byte anchor: market price model inputs; per-turn drift fn behind overlay thunk 0x181F:0x9A4 [TBD]; values are primary [B]; goods index = PowerRecord market arrays order [B]
...
#### `@ORDERS` — csv (13 row(s))
> byte anchor: unit order list + key letters; UnitRecord +0x05 candidate (0x2D='-'=No Orders) [B present]
...
#### `@LEVELS` — csv (5 row(s))
> byte anchor: native settlement size tiers (Camp/Village/City/...)
...
> byte anchor: native tribes; order = PowerRecord idx 4..11 [B]; color col verified via capital-raze note (Aztec=149, Inca=97) DATA_MODEL [B]

| name | singular | treasure | tech_level | color |
|---|---|---|---|---|
...
> byte anchor: Congress acquisition weights by era; PowerRecord +0x07 acquired bitmask [B]; selection fn TBD
